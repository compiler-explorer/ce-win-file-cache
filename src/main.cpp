...
{
public:
    CompilerCacheService() 
        : Fsp::Service(L"CompilerCacheFS")
        , filesystem_()
        , host_(filesystem_)
    {
    }
...
protected:
    NTSTATUS OnStart(ULONG argc, PWSTR* argv) override
    {
        ...
        {
            std::wcerr << L"Failed to initialize filesystem. Status: 0x" 
        ...
        if (!NT_SUCCESS(result))
        {
            std::wcerr << L"Failed to set compiler paths. Status: 0x" 
                       << std::hex << result << std::endl;
            return result;
        }
        ...
        if (!NT_SUCCESS(result))
        {
            std::wcerr << L"Failed to mount filesystem at " << mount_point 
                       << L". Status: 0x" << std::hex << result << std::endl;
            return result;
        }
...
int wmain(int argc, wchar_t** argv)
{
    ...
    if (!NT_SUCCESS(result))
    {
        std::wcerr << L"Failed to initialize WinFsp. Status: 0x" 
                   << std::hex << result << std::endl;
        return 1;
    }
