...
NTSTATUS HybridFileSystem::GetSecurityByName(
    PWSTR FileName,
...
NTSTATUS HybridFileSystem::Open(
    PWSTR FileName,
    ...
    OpenFileInfo* OpenFileInfo)
{
    ...
    *PFileDesc = file_desc;
    ...
    OpenFileInfo->FileInfo.FileSize = 
        ((UINT64)file_info.nFileSizeHigh << 32) | (UINT64)file_info.nFileSizeLow;
...
NTSTATUS HybridFileSystem::Read(
    PVOID FileNode,
    PVOID FileDesc,
    PVOID Buffer,
    UINT64 Offset,
...
NTSTATUS HybridFileSystem::GetFileInfo(
    PVOID FileNode,
    PVOID FileDesc,
    FileInfo* FileInfo)
{
...
NTSTATUS HybridFileSystem::ReadDirectory(
    PVOID FileNode,
    PVOID FileDesc,
    PWSTR Pattern,
    PWSTR Marker,
...
{
    auto* file_desc = static_cast<FileDescriptor*>(FileDesc);
    return BufferedReadDirectory(&file_desc->dir_buffer,
        FileNode, FileDesc, Pattern, Marker, Buffer, Length, PBytesTransferred);
}

NTSTATUS HybridFileSystem::ReadDirectoryEntry(
    PVOID FileNode,
    PVOID FileDesc,
    PWSTR Pattern,
    PWSTR Marker,
