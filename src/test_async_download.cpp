...
public:
    ...
    static void runBasicTest()
    {
        ...
        std::vector<std::wstring> test_files = {
            L"test_files/small.txt",
            L"test_files/medium.txt", 
            L"test_files/large.txt"
        };
        ...
        for (const auto& file : test_files)
        {
            ...
            download_manager.queueDownload(
                virtual_path,
                file,
                nullptr, // No cache entry for this test
                CachePolicy::ALWAYS_CACHE,
                [&completed_count, &failed_count, vpath = virtual_path](NTSTATUS status, const std::wstring& error) {
        ...
        while (completed_count + failed_count < static_cast<int>(test_files.size()))
        {
            ...
            std::cout << "\rPending: " << download_manager.getPendingCount() 
                      << ", Active: " << download_manager.getActiveCount() 
        ...
        for (const auto& file : test_files)
        {
            ...
            {
                std::wcout << L"  ✓ " << virtual_path << L" is cached (" 
                          << content->size() / 1024 << L" KB)" << std::endl;
            }
            else
            {
