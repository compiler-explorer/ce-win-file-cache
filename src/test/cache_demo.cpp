...
void runCacheDemo()
{
    ...
    // Test files to load
    std::vector<std::wstring> test_files = { L"/msvc-14.40/bin/cl.exe", L"/msvc-14.40/include/iostream", L"/ninja/ninja.exe" };

    std::cout << "\n1. INITIAL LOAD (Cache Miss - Loading from 'Network')\n" << std::endl;
    std::cout << std::setw(40) << std::left << "File" << std::setw(12) << "Size" << std::setw(12) << "Load Time"
              << "Status" << std::endl;
    ...
    for (const auto &path : test_files)
    {
        ...
        std::cout << std::setw(12) << formatBytes(content.size()) << std::setw(12) << (std::to_string(duration) + " μs")
                  << (content.empty() ? "FAILED" : "Cached") << std::endl;
    }

    std::cout << "\nTotal network load time: " << total_network_time << " μs" << std::endl;
    std::cout << "Cache status: " << cache.getCachedFileCount() << " files, " << formatBytes(cache.getCacheSize())
              << " total" << std::endl;
    ...
    for (const auto &path : test_files)
    {
        ...
        double speedup = network_times[file_idx] > 0 ? static_cast<double>(network_times[file_idx]) / duration : 0;

        std::wcout << std::setw(40) << std::left << path;
        std::cout << std::setw(12) << formatBytes(content.value_or(std::vector<uint8_t>()).size()) << std::setw(12)
                  << (std::to_string(duration) + " μs") << std::fixed << std::setprecision(1) << speedup << "x faster"
                  << std::endl;
    ...
    std::cout << "Overall speedup: " << std::fixed << std::setprecision(1)
              << (total_network_time > 0 ? static_cast<double>(total_network_time) / total_cache_time : 0) << "x faster"
              << std::endl;
    ...
    for (const auto &path : test_files)
    {
        std::wcout << "  " << path << ": " << (cache.isFileInMemoryCache(path) ? "YES" : "NO") << std::endl;
    }
