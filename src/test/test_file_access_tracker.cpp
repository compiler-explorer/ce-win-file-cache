...
int main()
{
    ...
    tracker.recordAccess(L"/msvc-14.40/bin/cl.exe", L"\\\\server\\msvc\\14.40\\bin\\cl.exe", 
                        1024 * 1024, FileState::CACHED, true, true, 5.5, L"always_cache");
    
    tracker.recordAccess(L"/msvc-14.40/include/iostream", L"\\\\server\\msvc\\14.40\\include\\iostream",
                        8192, FileState::CACHED, true, false, 2.1, L"on_demand");
    
    tracker.recordAccess(L"/ninja/ninja.exe", L"\\\\server\\tools\\ninja.exe",
                        512 * 1024, FileState::NETWORK_ONLY, false, false, 15.8, L"never_cache");
    
    // Record multiple accesses to same file
    tracker.recordAccess(L"/msvc-14.40/bin/cl.exe", L"\\\\server\\msvc\\14.40\\bin\\cl.exe",
                        1024 * 1024, FileState::CACHED, true, true, 1.2, L"always_cache");
    
    tracker.recordAccess(L"/msvc-14.40/bin/cl.exe", L"\\\\server\\msvc\\14.40\\bin\\cl.exe",
                        1024 * 1024, FileState::CACHED, true, true, 0.8, L"always_cache");
    ...
    autoTracker.recordAccess(L"/test/file1.txt", L"\\\\server\\test\\file1.txt",
                           1024, FileState::FETCHING, false, false, 10.0, L"on_demand");
    ...
    {
        std::wcerr << L"❌ Second manual reporting didn't generate additional reports (found " << reportCount << L" files)" << std::endl;
        return 1;
    }
