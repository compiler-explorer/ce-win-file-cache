...
int testRealFilesystem()
{
    ...
    if (!root_contents.empty())
    {
        ...
        for (const auto* node : root_contents)
        {
            if (count >= 5) break; // Show first 5 items only
            std::wcout << L"  - " << node->name;
            if (node->isDirectory()) 
    ...
    if (tree_file.is_open())
    {
        ...
        tree_file << "Performance: " << std::fixed << std::setprecision(1) << (total_nodes / (duration / 1000.0)) << " nodes/second" << std::endl;
