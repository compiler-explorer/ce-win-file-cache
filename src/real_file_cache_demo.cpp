...
void runRealFileDemo() {
    ...
    std::cout << std::setw(20) << std::left << "File Type" 
              << std::setw(15) << "Size" 
    ...
    for (const auto& [path, desc] : test_files) {
        ...
        std::cout << std::setw(20) << std::left << desc
                  << std::setw(15) << formatBytes(content.size())
                  << std::setw(15) << (std::to_string(duration) + " μs")
                  << (content.empty() ? "FAILED" : "Loaded & Cached") << std::endl;
    }
    
    std::cout << "\nCache status: " << cache.getCachedFileCount() << " files, " 
              << formatBytes(cache.getCacheSize()) << " in memory" << std::endl;
    ...
    for (int iteration = 1; iteration <= 5; iteration++) {
        ...
        for (const auto& [path, desc] : test_files) {
            ...
            if (cached.has_value()) {
                double speedup = first_load_times[idx] > 0 ? 
                    static_cast<double>(first_load_times[idx]) / duration : 0;
                
                std::cout << std::setw(20) << std::left << desc
                          << std::setw(15) << formatBytes(cached->size())
                          << std::setw(15) << (std::to_string(duration) + " μs")
            ...
            if (desc == "Text file") {
                ...
                for (size_t i = 0; i < bytes_to_show; i++) {
                    std::cout << std::hex << std::setw(2) << std::setfill('0') 
                              << static_cast<int>((*cached)[i]) << " ";
                    if ((i + 1) % 16 == 0) std::cout << "\n  ";
                }
            }
            std::cout << std::dec << "\n  [" << (cached->size() - bytes_to_show) 
                      << " more bytes cached in memory]" << std::endl;
        }
        std::cout << std::endl;
    }
