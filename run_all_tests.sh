...
# Script to build and run all test programs on macOS
# 
# This script will:
...
#
# Examples:
#   ./run_all_tests.sh              # Full build and test run
#   ./run_all_tests.sh --clean      # Clean build and test run
#   ./run_all_tests.sh --quick      # Skip configuration, just build and test
...
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            echo "macOS Test Runner for CE Win File Cache"
            echo
...
echo -e "${BLUE}=== macOS Test Runner ===${NC}"
...
if [[ "$QUICK_BUILD" == true && -d "$BUILD_DIR" ]]; then
    ...
    cd "$PROJECT_ROOT"
    cmake -S . -B "$BUILD_DIR" -DBUILD_CACHE_TEST=ON -DENABLE_METRICS=ON
fi
echo
